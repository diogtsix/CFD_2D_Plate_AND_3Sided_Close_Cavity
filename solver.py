...
class Solver_implicit:
    def __init__(self, grid_nodes_x, grid_nodes_y, grid_u_velocity, grid_v_velocity, dx, dy):
        ...
        self.grid_u_velocity = grid_u_velocity.astype(np.float64)
        self.grid_v_velocity = grid_v_velocity.astype(np.float64)
        self.dx = dx
        self.dy = dy

    def u_i_plus_1_system_eq(self):
        nodes_on_y_axis = np.round(self.grid_nodes_y.shape[0]).astype(int)
        nodes_on_x_axis = np.round(self.grid_nodes_x.shape[1]).astype(int)
        LHS_constants = np.zeros((nodes_on_y_axis - 1, 1), dtype=np.float64)
        lower_diagonal = np.ones((nodes_on_y_axis - 2, 1), dtype=np.float64)
        diagonal = np.ones((nodes_on_y_axis - 1, 1), dtype=np.float64)
        upper_diagonal = np.ones((nodes_on_y_axis - 2, 1), dtype=np.float64)
    ...
    def v_i_plus_1(self, i, nodes_on_y_axis):
        for jj in range(1, nodes_on_y_axis - 1):
            Lamda = -(2 * (self.dy / self.dx)) * (self.grid_u_velocity[jj, i + 1] - self.grid_u_velocity[jj, i]) - (
                        self.grid_v_velocity[jj + 1, i] - self.grid_v_velocity[jj, i])
